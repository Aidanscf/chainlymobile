...
const NATIVE_ALIASES = {
  ...
  // TEMP_DISABLED: stub expo-notifications on native to avoid Firebase crash
  'expo-notifications': path.resolve(
    __dirname,
    './polyfills/native/notifications.native.js'
  ),
};
...
config.resolver.resolveRequest = (context, moduleName, platform) => {
  try {
    ...
    // Standard resolution for polyfills to prevent cycles
    const isPolyfillOrigin =
      context.originModulePath.includes(`${path.sep}polyfills${path.sep}native`) ||
      context.originModulePath.includes(`${path.sep}polyfills${path.sep}web`) ||
      context.originModulePath.includes(`${path.sep}polyfills${path.sep}shared`);
